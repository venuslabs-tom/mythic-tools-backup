...
$(document).ready(async () => {
  ...
  try {
    ...
    tiles = data.tiles;
    mythNames = data.mythNames;
    console.log("Success: Loading local map.json");
  } catch (e) {
    console.log("No local map found, generating new one.");
    tiles = generate(
      LAND_CHARACTERS[Math.floor(Math.random() * LAND_CHARACTERS.length)],
      LAND_LANDSCAPES[Math.floor(Math.random() * LAND_LANDSCAPES.length)]
    );
  }
  ...
  /**
   * HEX INTERACTION: Myth name updates
   */
  ...
  /**
   * HEX INTERACTION: Listen to realm config updates
   */
  $(document).on("change", ".config input", updateCfg);

  /**
   * HEX INTERACTION: Edit a tile
   */
  ...
  /**
   * UI CONTROLS: Toggle player mode
   */
  ...
  /**
   * UI CONTROLS: Toggle grayscale
   */
  $(document).on("change", "#grayscale-toggle", () => {
    regenerate(tiles);
  });

  /**
   * HOVER LOGIC: Highlight myths
   */
  ...
  // This must stay INSIDE the $(document).ready block
  ...
  if (elem && typeof Panzoom !== 'undefined') {
    ...
    // Make it global so you can test it in the F12 console
    window.pz = pz;
    console.log("Panzoom successfully attached to #map-container");
  } else if (typeof Panzoom === 'undefined') {
    console.error("Panzoom library is missing! Add the CDN link to your index.html.");
  }

}); // This is the FINAL closing bracket for the ready function.