...
const UI = {
  ...
  initializeUi: () => {
    ...
    $(document).on("click", ".close-dialog", (ev) => {
      ...
      if (d.is("form")) d.trigger("submit");
      else d.slideUp("fast", () => d.remove());
      ev.preventDefault();
      ev.stopPropagation();
    });
  },
  ...
  Confirm: (title, content, options = {}) => {
    ...
    `);
    const dialog = UI.Dialog(title, content, { classes: options.classes });
...
$(document).ready(UI.initializeUi);
